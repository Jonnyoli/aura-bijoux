...
import { Heart, Shrub as Share2, Star, Truck, Shield, Ruler, Info, Check, X, MessageSquare, User as UserIcon, RefreshCcw, ShoppingBag } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16">
        ...
        <div className="space-y-6">
          <div className="flex gap-2 p-1 bg-white dark:bg-black/50 border border-slate-100 dark:border-white/10 rounded-2xl transition-all">
            <button
              onClick={() => setViewMode('studio')}
              className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${viewMode === 'studio' ? 'bg-white dark:bg-black text-pink-500 shadow-sm border border-slate-100 dark:border-white/10' : 'text-slate-400 hover:text-pink-500 transition-colors'} `}
            >
              Estúdio
            </button>
            <button
              onClick={() => setViewMode('context')}
              className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${viewMode === 'context' ? 'bg-pink-500 text-white shadow-lg' : 'text-gray-400 hover:text-gray-600'} `}
            >
              Uso Real (Escala)
            </button>
          </div>
          ...
          <div
            className={`aspect-[4/5] rounded-[2.5rem] overflow-hidden bg-white dark:bg-black shadow-inner group relative cursor-zoom-in transition-all duration-700 ${viewMode === 'context' ? 'ring-4 ring-pink-500/20' : ''} `}
          ...
          >
            {allMedia[selectedMedia]?.type === 'video' ? (
              ...
              <div
                className={`w-full h-full transition-all duration-700 ease-out ${viewMode === 'context' ? 'scale-110 blur-[1px] brightness-90' : ''} `}
                style={isZoomed && viewMode === 'studio' ? {
              ...
              >
                {viewMode === 'context' && (
                  <div className="absolute inset-0 flex items-center justify-center p-8">
                    <div className="relative w-full h-full border-2 border-white/30 rounded-3xl flex items-center justify-center animate-pulse-slow">
                      <div className="absolute top-4 left-4 text-white text-[8px] font-bold uppercase tracking-[0.2em] bg-black/40 px-3 py-1.5 rounded-full backdrop-blur-md">Escala Simulada</div>
                      <img
                        src={allMedia[selectedMedia]?.url}
                        alt="Scale View"
                        className="w-2/3 h-2/3 object-contain drop-shadow-[0_20px_40px_rgba(0,0,0,0.4)] transform -rotate-6"
                      />
                    </div>
                  </div>
                )}
        ...
        <div className="flex flex-col">
          <div className="mb-2 flex items-center justify-between">
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                className={`p-2 rounded-full border border-slate-100 dark:border-white/10 shadow-sm transition-all duration-300 ${isWishlisted ? 'text-pink-500 bg-pink-50 dark:bg-pink-900/30 border-pink-100 dark:border-pink-900' : 'text-slate-400 bg-white dark:bg-black hover:text-pink-500'} `}
              >
          ...
          <div className="bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 shadow-sm rounded-[2.5rem] transition-all p-8 relative overflow-hidden mb-8">
            <div className="absolute top-0 right-0 w-40 h-40 bg-pink-100/40 dark:bg-pink-900/20 rounded-full -mr-16 -mt-16 blur-2xl"></div>
            <div className="flex items-center text-[10px] font-bold uppercase tracking-[0.2em] relative z-10 mb-4 pb-4 border-b border-slate-200 dark:border-white/10">
              {product.stock > 0 ? (
            ...
            <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed font-light relative z-10 mb-6">{product.description}</p>

            {(product.category === 'Anéis' || product.category === 'Colares' || product.category === 'Pulseiras') && (
              <div className="relative z-10 flex border-t border-slate-200 dark:border-white/10 pt-4">
                <button
                  onClick={() => setIsSizeGuideOpen(true)}
                  className="flex items-center justify-center gap-2 w-full bg-white dark:bg-black border border-slate-200 dark:border-white/10 rounded-xl py-3 text-[10px] font-bold uppercase tracking-[0.2em] text-gold hover:bg-gold hover:text-white hover:border-gold transition-all shadow-sm"
                >
                  <Ruler size={16} /> Ver Guia de Tamanhos
                </button>
              </div>
            )}
          </div>
          ...
          {product.stock === 0 && (
            <div className="bg-white dark:bg-black/50 border border-slate-100 dark:border-white/5 p-6 rounded-[2rem] transition-all shadow-inner">
              ...
              {restockStatus === 'success' ? (
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    className="flex-grow bg-white dark:bg-black border border-slate-100 dark:border-white/10 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 ring-pink-100 dark:ring-pink-900/30 transition-all dark:text-white placeholder:text-slate-300"
                    value={restockEmail}
          ...
          {product.stock > 0 && (
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row items-center gap-4">
                <div className="flex items-center border border-slate-100 dark:border-white/10 rounded-2xl overflow-hidden bg-slate-50/50 dark:bg-white/5 p-1 w-full sm:w-auto">
                  <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="flex flex-col gap-2 p-4 bg-white dark:bg-black/80 backdrop-blur-md border border-slate-100 dark:border-white/10 shadow-luxury rounded-2xl transition-all">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-gold/10 text-gold rounded-2xl flex items-center justify-center shrink-0 group-hover:bg-gold group-hover:text-white transition-colors duration-500">
                      <Truck size={18} />
                    </div>
                    <div className="text-[10px] uppercase font-bold tracking-wider">
                      <p className="text-slate-900 dark:text-white">Envio Hoje</p>
                      <p className="text-gold">Premium Dispatch</p>
                    </div>
                  </div>
                  <div className="mt-2 pt-2 border-t border-slate-50 dark:border-white/5">
                    <p className="text-[10px] text-slate-500">
                      Encomende em <span className="text-slate-900 dark:text-white font-black">{timeLeft.hours}h {timeLeft.minutes}m</span> para envio hoje.
                    </p>
                  </div>
                </div>

                <div className="flex items-center gap-4 p-4 bg-white dark:bg-gray-800 border-2 border-slate-50 dark:border-gray-700 rounded-3xl shadow-sm hover:border-pink-100 transition-all group">
                  <div className="w-12 h-12 bg-pink-50 dark:bg-pink-900/20 text-pink-500 rounded-2xl flex items-center justify-center shrink-0 group-hover:rotate-12 transition-transform duration-500">
                    <RefreshCcw size={20} />
                  </div>
                  <div>
                    <p className="font-bold text-[10px] text-slate-900 dark:text-white uppercase tracking-wider">Troca Grátis</p>
                    <p className="text-[10px] text-slate-400">14 dias sem complicações</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="mt-12">
            <div className="flex border-b border-gray-100 dark:border-gray-700 mb-8 overflow-x-auto scrollbar-hide">
              <button
                ...
                className={`px-8 py-4 text-xs font-bold uppercase tracking-[0.2em] transition-all relative whitespace-nowrap ${activeTab === 'description' ? 'text-pink-500' : 'text-slate-400 hover:text-slate-600'} `}
              >
                Detalhes da Peça
              ...
              <button
                ...
                className={`px-8 py-4 text-xs font-bold uppercase tracking-[0.2em] transition-all relative whitespace-nowrap ${activeTab === 'material' ? 'text-pink-500' : 'text-slate-400 hover:text-slate-600'} `}
              >
                Materiais & Cuidados
              ...
              <button
                ...
                className={`px-8 py-4 text-xs font-bold uppercase tracking-[0.2em] transition-all relative whitespace-nowrap ${activeTab === 'reviews' ? 'text-pink-500' : 'text-slate-400 hover:text-slate-600'} `}
              >
            ...
            <div className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed min-h-[120px] animate-fade-in">
              ...
              {activeTab === 'reviews' && (
                <div className="space-y-10 animate-fade-in">
                  ...
                  {/* Add Review Form */}
                  <div className="bg-white dark:bg-black border border-slate-100 dark:border-white/10 rounded-[2.5rem] p-8 shadow-inner transition-all">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-black border-t border-slate-100 dark:border-white/10 p-4 md:hidden z-40 flex items-center gap-4 animate-slide-up shadow-[0_-4px_20px_-5px_rgba(0,0,0,0.1)]">
