...
import { Trash2, ArrowLeft, ShoppingBag, Plus, Minus, CreditCard, X } from 'lucide-react';
import { useApp } from '../store';

const Cart: React.FC = () => {
  const { cart, updateCartQuantity, removeFromCart, vouchers, appliedVoucher, setAppliedVoucher } = useApp();
  ...
  const handleApplyVoucher = () => {
    ...
    const v = vouchers.find(v => v.code === voucherCode.toUpperCase());
    if (!v) {
      setVoucherError('Voucher não encontrado.');
      return;
    }
    if (!v.isActive) {
      setVoucherError('Voucher inativo.');
      return;
    }
    if (v.minPurchase && subtotal < v.minPurchase) {
      setVoucherError(`Compra mínima de ${v.minPurchase}€ não atingida.`);
      return;
    }
    if (v.maxUsage && v.usageCount >= v.maxUsage) {
      setVoucherError('Voucher esgotado.');
      return;
    }
      ...
      <div className="min-h-[70vh] flex items-center justify-center px-4 py-12">
        <div className="max-w-2xl w-full text-center space-y-10 animate-fade-in">
          <div className="relative inline-block">
            ...
            <div className="relative bg-white dark:bg-black border border-slate-100 dark:border-white/10 shadow-soft dark:shadow-luxury transition-all p-12 rounded-full inline-flex items-center justify-center">
          ...
          <div className="pt-12">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500 font-bold mb-6">Inspiração para si</p>
            <div className="grid grid-cols-3 gap-4 max-w-md mx-auto opacity-60 hover:opacity-100 transition-opacity duration-500">
              <div className="aspect-square rounded-2xl overflow-hidden bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 box-content">
                <img src="https://images.unsplash.com/photo-1515562141207-7a88fb7ce33e?q=80&w=400&auto=format&fit=crop" alt="Inspiration" className="w-full h-full object-cover hover:scale-110 transition-transform duration-700" referrerPolicy="no-referrer" />
              </div>
              <div className="aspect-square rounded-2xl overflow-hidden bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 box-content">
                <img src="https://images.unsplash.com/photo-1535633302703-b070b4628bc5?q=80&w=400&auto=format&fit=crop" alt="Inspiration" className="w-full h-full object-cover hover:scale-110 transition-transform duration-700" referrerPolicy="no-referrer" />
              </div>
              <div className="aspect-square rounded-2xl overflow-hidden bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 box-content">
                <img src="https://images.unsplash.com/photo-1605100804763-247f67b3f4ad?q=80&w=400&auto=format&fit=crop" alt="Inspiration" className="w-full h-full object-cover hover:scale-110 transition-transform duration-700" referrerPolicy="no-referrer" />
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="hidden sm:grid grid-cols-4 pb-4 border-b border-slate-100 dark:border-white/10 text-xs font-bold text-slate-400 uppercase tracking-widest">
          ...
          {cart.map((item) => (
            <div key={item.id} className="grid grid-cols-1 sm:grid-cols-4 items-center gap-6 pb-6 border-b border-slate-50 dark:border-white/5 group transition-colors">
              {/* Product Info */}
              <div className="col-span-1 sm:col-span-2 flex items-center gap-4">
                <Link to={`/product/${item.id}`} className="w-24 h-24 rounded-xl overflow-hidden bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 flex-shrink-0">
              ...
              <div className="flex justify-center">
                <div className="flex items-center border border-slate-100 dark:border-white/10 rounded-lg overflow-hidden bg-slate-50 dark:bg-white/5">
                  <button
        ...
        <div className="lg:col-span-1">
          <div className="bg-white dark:bg-black/50 border border-slate-100 dark:border-white/5 shadow-inner rounded-[2.5rem] p-8 transition-all sticky top-32">
            <h3 className="font-serif font-bold text-2xl mb-6 text-gray-900 dark:text-white">Resumo</h3>

            <div className="space-y-4 mb-6">
              {/* Free Shipping Progress Bar */}
              <div className="bg-white dark:bg-black border border-slate-100 dark:border-white/10 p-4 rounded-xl shadow-sm transition-all">
                ...
                <div className="w-full bg-slate-100 dark:bg-white/10 h-2 rounded-full overflow-hidden mb-2">
                  <div
              ...
              {appliedVoucher && (
                <div className="flex justify-between text-pink-500 font-bold">
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setAppliedVoucher(null)} className="text-gray-400 hover:text-red-500">
                      <X size={14} />
                    </button>
                  </div>
            ...
            <div className="border-t border-slate-200 dark:border-white/10 pt-6 mb-8 flex justify-between items-end">
            ...
            <div className="mt-8 pt-8 border-t border-slate-200 dark:border-white/10">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg px-3 py-2 text-sm outline-none transition-all dark:text-white"
                />
                <button onClick={handleApplyVoucher} disabled={!!appliedVoucher} className="bg-slate-900 dark:bg-gold dark:text-black text-white px-4 py-2 rounded-lg text-sm font-bold hover:opacity-90 transition-all disabled:opacity-50">Aplicar</button>
              </div>
              {voucherError && <p className="text-red-500 text-xs mt-2 font-bold">{voucherError}</p>}
            </div>
          </div>
        </div>
      </div>
