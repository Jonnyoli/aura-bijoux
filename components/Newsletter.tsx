...
const Newsletter: React.FC = () => {
    ...
    return (
        <section className="relative py-24 overflow-hidden bg-white dark:bg-black border-t border-slate-100 dark:border-white/5">
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="bg-slate-900 dark:bg-white/5 backdrop-blur-md border border-white/10 rounded-[3rem] p-8 md:p-16 text-center max-w-4xl mx-auto overflow-hidden shadow-2xl">
                    {status === 'success' ? (
                        <div className="animate-fade-in space-y-6 py-8">
                            ...
                            <h2 className="text-3xl md:text-4xl font-serif font-bold text-slate-300 dark:text-slate-400 font-medium mb-2">Bem-vinda à Família Aura!</h2>
                        ...
                        <div className="animate-fade-in space-y-8">
                            ...
                            <div className="space-y-4">
                                <h2 className="text-4xl md:text-5xl font-serif font-bold text-slate-300 dark:text-slate-400 font-medium leading-tight">
                            ...
                            <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-4 max-w-xl mx-auto mt-10">
                                <input
                                    ...
                                    className="flex-grow bg-white/5 border border-white/10 text-slate-300 dark:text-slate-400 font-medium rounded-2xl px-6 py-4 outline-none focus:ring-2 ring-gold/30 transition-all placeholder:text-slate-500"
                                />
                                <button
                                    type="submit"
                                    disabled={status === 'loading'}
                                    className="bg-gold text-slate-900 dark:text-white px-8 py-4 rounded-2xl font-bold hover:bg-gold/90 transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-gold/20"
                                >
                                    {status === 'loading' ? (
